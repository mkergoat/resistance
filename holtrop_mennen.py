...
def hull_form_factor(T, L, B, LCB, Cp, C_stern):
    ...
    return c13 * (0.93 + c12 * (B / LR) ** 0.92497 * (0.95 - Cp) ** (-0.521448) * (1 - Cp + 0.0225 * LCB_midoL) ** (
        0.6906))
...
def friction_resistance(V, L, rho, mu, S, B, T, LCB, Cp, C_stern):
    """
    Computes the dimensional value of hull friction resistance
    """
    return makeDimensionnal(rho, V, S, CF_ITTC57(Re(V, L, rho, mu))) * hull_form_factor(T, L, B, LCB, Cp, C_stern)
